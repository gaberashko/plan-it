...
let contentContainer = document.querySelector(".content-wrapper") as HTMLDivElement;
...
class ProjectList {
    ...
    private save = (): void => {
        console.log(this);
        const serializable: {title:string, tasks:Task[]}[] = this.projects?.map((p) => ({
            title: p.title,
            tasks: p.tasks,
        }));
    ...
    private editProject(index: number): void {
        let titleInput = document.querySelector(`#editListName${index}`) as HTMLInputElement;

        let project: Project = new Project(titleInput.value, this.projects[index].tasks, this.save);
    ...
    public displayProjects(): void {
        let projectContainer = document.querySelector(".list-wrapper") as HTMLDivElement;
        if (
